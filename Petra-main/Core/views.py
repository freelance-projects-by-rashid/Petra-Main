...
from Core.models import Banner,Gallery_Image,Partners,Event,Review,Enquiry,Schools,Premium,Single,Double
...
def add_premium(request):
    if request.method == 'POST':
        ...
        try:
            # resized = resize(image,800)
            Premium.objects.create(Name=name, Image=image, Image2=image2, Description=desc)
...
def add_single(request):
    if request.method == 'POST':
        ...
        try:
            # resized = resize(image,800)
            Single.objects.create(Name=name, Image=image, Image2=image2, Description=desc)
...
def edit_single(request, single_id):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.warning(request, exception)
            return redirect('edit-single', premium_id=single.id)
